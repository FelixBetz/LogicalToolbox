...
public abstract class Formula {
   ...
   * @param assignment mapping between variables and truth values
   * @return {@code true} if the formula evaluates to {@link #VERUM}, {@code false} if it evaluates to {@link #FALSUM}
   * @throws IllegalArgumentException if the given assignment is not complete
   */
  public abstract boolean evaluate(final Map<Variable, Boolean> assignment) throws IllegalArgumentException;

  /**
   * Test if two formulas are syntactically equal. Note: This is not a test for semantical equality.
   * @param other formula to compare with
   * @return {@code true} if the given formula is syntactically equal to this formula, {@code false} otherwise
   */
  ...
  /**
   * Tests if the formula is an atomic formula. An atomic formula is a variable, verum or falsum.
   * @return {@code true} if formula is atomic, {@code false} otherwise
   */
  public abstract boolean isAtomicFormula();

  /**
   * Tests if the formula is a literal. A literal is an atomic formula or the negation of an atomic formula.
   ...
   * (s1 AND s2)). For every other formula, an IllegalArgumentException will be
   * thrown.
   * @param f The Formula on which to apply the De Morgan rule
